...
<head>
    ...
    <style>
        ...
        .sandbox-container, .mathjax-renderer {
            background-color: var(--card-bg);
            border-radius: 8px;
            padding: 20px;
...
printf("%d", c);</code></pre><p>因此，程式會輸出 `3`。</p></div>
        ...
        <aside class="interactive-panel">
            <div class="interactive-panel-inner">
                <div class="sandbox-container">
                    <h3>▶ 程式碼沙箱 (Emscripten / WASM)</h3>
                    <p style="font-size: 0.9em; margin-top: -10px; opacity: 0.8;">點擊左側「運行示例」或直接編輯，程式碼將透過後端編譯為 WebAssembly 在瀏覽器中執行。</p>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // --- Code Sandbox Logic with Emscripten Backend ---
            runCodeBtn.addEventListener('click', async () => {
                ...
                try {
                    // **重要**: 請將下面的 URL 修改為您 Node.js 後端服務的正確位址
                    const backendUrl = 'http://127.0.0.1:3000/compile';
                    // 如果後端部署在不同機器，請使用 'http://<您的伺服器IP位址>:3000/compile'
                    ...
                    // Create Blob objects and URLs
                    const jsBlob = new Blob([mainJsText], { type: 'application/javascript' });
                    const wasmBlob = new Blob([mainWasmBinary], { type: 'application/wasm' });
                    const jsURL = URL.createObjectURL(jsBlob);
                    const wasmURL = URL.createObjectURL(wasmBlob);

                    outputArea.textContent = '執行中...\n\n▶ 執行結果:\n';

                    // Cleanup previous script and Module to allow re-running
                    const oldScript = document.getElementById('emcc-script');
                    if (oldScript) {
                        oldScript.remove();
                    }
                    delete window.Module;

                    // Define the Module object for Emscripten glue code
                    window.Module = {
                        wasmBinary: mainWasmBinary,
                        print: (text) => {
                        ...
                        onRuntimeInitialized: () => {
                           setTimeout(() => {
                               if (!outputArea.textContent.endsWith('\n\n--- 執行完畢 ---')) {
                    ...
                    // Dynamically add and run the compiled JS glue code
                    const script = document.createElement('script');
                    script.id = 'emcc-script';
                    script.src = jsURL;
