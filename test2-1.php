...
<head>
    ...
    <title>C/C++ 第二章 互動練習題 (詳細解說)</title>
...
<body>
    <div class="container">
        <main class="tutorial-content">
            <h1>C/C++ 第二章：變數、資料型別與I/O</h1>
            <p>本頁面提供一系列關於 C/C++ 語言基礎的互動練習題，涵蓋變數命名、資料型別、記憶體概念、以及標準輸出入操作。請仔細閱讀每個題目，並選擇最合適的答案。部分題目附有程式碼片段，您可以點擊「運行示例」按鈕，將程式碼載入到右側的沙箱中實際執行和測試，以幫助理解。每個問題的詳解中都會提供詳細的步驟分析。</p>
                ...
                <div id="q3" class="quiz-card">
                    ...
                    <div class="explanation">
                        ...
                        <ul>
                            ...
                            <li><b>(C) byte:</b> <code>byte</code> 不是 C++ 標準的關鍵字或基本資料型態。雖然一個位元組 (byte) 是記憶體的基本單位，且 <code>char</code> 通常佔用一個位元組，但 <code>byte</code> 本身並非型別關鍵字。然而，題目問「不是整數資料型態」，而 <code>double</code> 明顯不是。</li>
                            <li><b>(D) int:</b> 標準的整數資料型態。</li>
                        </ul>
                        <p>在選項中，<code>double</code> 是唯一明確的非整數資料型態。</p>
                ...
                <div id="q24" class="quiz-card">
                    ...
                    <div class="explanation">
                        ...
                        <ul>
                            ...
                            <li>(D) 0 ~ 4,294,967,295：這通常是 4 bytes (32-bit) 無符號整數 (<code>unsigned int</code>) の範圍 (2<sup>32</sup> - 1)。</li>
                        </ul>
                        <h4>✓ 正確答案</h4> <p>本題的正確答案是： (C)</p>
                    </div>
...
// printf("%d", result); // 結果為 4</code></pre>
                ...
                <div id="q31" class="quiz-card">
                    <div class="preamble-text"><p>A閱讀下文，回答第 31~32 題</p></div>
                    <div class="preamble-code"><pre><code class="language-c">1 #define Value1  100
2 #define Value2 (Value1 - 1)
3 const  int  Value3;
4 int CheckValue = 0;
5 Value3 = Value2;
6 CheckValue = Value1 + Value3;</code></pre></div>
                    <h3>31. 小芳在一個原本可以編譯(Compile)成功的程式中，在 main( )主程式內再加入行號 1 至行號 6 的程式碼，但加入後發 編譯錯誤的情況。小芳刪除行號 1 至行號 5 中的哪一個部分後，可以使程式編譯成功？</h3>
                    ...
                    <div class="explanation">
                        ...
                        <ul>
                            ...
                            <li>行 3: <code>const int Value3;</code> - 宣告一個整數型別的<b>常數</b> <code>Value3</code>。<b>關鍵問題：</b><code>const</code> 變數必須在宣告時進行初始化。這裡 <code>Value3</code> 被宣告為 <code>const</code> 但沒有立即給予初始值，這本身就是一個編譯錯誤。</li>
                            <li>行 5: <code>Value3 = Value2;</code> - 試圖將 <code>Value2</code> (即 99) 賦值給 <code>Value3</code>。由於 <code>Value3</code> 被宣告為 <code>const</code>，一旦它被（正確地）初始化後，其值就不能再被修改。在此情況下，即使它能被延遲初始化（某些編譯器對全域或靜態 const 可能有不同規則，但在函數內部通常不行），這也是一次賦值操作。</li>
                        </ul>
                        <p>主要的編譯錯誤來自於行 3（未初始化的 const 變數）和行 5（嘗試對 const 變數賦值）。</p>
                        <p>要使程式編譯成功，最直接的方法是讓 <code>Value3</code> 可以被賦值，並且解決未初始化 const 的問題。如果刪除行 3 中的 <code>const</code> 關鍵字，<code>Value3</code> 就會變成一個普通的 <code>int</code> 變數。這樣，行 3 變為 <code>int Value3;</code> (宣告一個變數，其值未定)，然後行 5 的賦值 <code>Value3 = Value2;</code> 就會變得合法。</p>
                        <p>分析選項：</p>
                        <ul>
                            <li>(A) 刪除 <code>(Value1 - 1)</code>：這會使 <code>Value2</code> 的定義不完整，無法解決 <code>Value3</code> 的問題。</li>
                            <li>(B) 刪除 <code>Value3 = Value2;</code>：這會解決行 5 的賦值錯誤，但行 3 的 <code>const int Value3;</code> 仍然是一個未初始化的常數。在 C/C++ 中，未初始化的 const 變數本身就是一個錯誤，並且在行 6 使用它也會導致問題。</li>
...
5 Value3 = Value2;
6 CheckValue = Value1 + Value3;</code></pre></div>
                    <h3>32. 程式修正後，當程式執行完行號 6 的時候，CheckValue 的值為下列何者？</h3>
                    <button class="run-example-btn" data-code-id="q31_32-code">運行示例 (Q31&32)</button>
                    <div class="quiz-options" data-correct="B">
